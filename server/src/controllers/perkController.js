...
export async function getAllPerksPublic(req, res, next) {
  try {
    ...
    try {
      ...
      // If populate fails due to malformed createdBy values (e.g. stored as username string)
      // retry the query without populate to avoid crashing the endpoint.
      // This surfaces perks but without populated user info.
      if (populateErr.name === 'CastError' && String(populateErr.message).includes('User')) {
        perks = await Perk
          .find(query)
          .sort({ createdAt: -1 })
          .lean();
